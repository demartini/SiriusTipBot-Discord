...
exports.tip = {
  usage: '<subcommand>',
  description:
    'Tip a given user with an amount of SIRX or perform wallet specific operations.',
  process: async function(bot, msg, suffix) {
    ...
    let subcommand = words.length >= 2 ? words[1] : 'help'
    let channelwarning =
      'Please use <#' + spamchannel + '> or DMs to talk to bots.'
    let MultiorRole = false
    switch (subcommand) {
      case 'help':
      ...
      case 'deposit':
        ...
        break
      case 'withdraw':
        privateOrSandboxOnly(msg, channelwarning, doWithdraw, [
          tipper,
          words,
          helpmsg
        ])
...
exports.multitip = {
  usage: '<subcommand>',
  description:
    'Tip multiple users simultaneously for the same amount of SIRX each.',
  process: async function(bot, msg, suffix) {
    let tipper = msg.author.id.replace('!', '')
    let words = msg.content
      .trim()
...
exports.roletip = {
  usage: '<subcommand>',
  description: 'Tip all users in a specified role an amount of SIRX.',
  process: async function(bot, msg, suffix) {
    let tipper = msg.author.id.replace('!', '')
    let words = msg.content
      .trim()
    ...
    let subcommand = words.length >= 2 ? words[1] : 'help'
    let channelwarning = `Please use <#${spamchannel}> or DMs to talk to bots.`
    let MultiorRole = true
    switch (subcommand) {
      case 'help':
        privateOrSandboxOnly(msg, channelwarning, doHelp, [helpmsg])
...
exports.tips = {
  usage: '',
  description:
    'Lists all available TipBot commands with brief descriptions for each one.',
  process: async function(bot, msg, suffix) {
    msg.reply(helpmsg)
  }
}
...
function doBalance(message, tipper) {
  sirius.getBalance(tipper, 1, function(err, balance) {
    if (err) {
      message
...
function doWithdraw(message, tipper, words, helpmsg) {
  ...
  let address = words[2]
  let amount = getValidatedAmount(words[3])

  if (amount === null) {
    message
      .reply('Invalid amount of SIRX specified... Cannot withdraw SIRX.')
      .then(message => message.delete(5000))
    return
  }
...
function doTip(bot, message, tipper, words, helpmsg, MultiorRole) {
  ...
  if (amount === null) {
    return message
      .reply('Invalid amount of SIRX specified...')
      .then(message => message.delete(5000))
  }
...
function doMultiTip(bot, message, tipper, words, helpmsg, MultiorRole) {
  ...
  if (!userIDs) {
    message
      .reply('Sorry, I could not find the user you are trying to tip...')
      .then(message => message.delete(5000))
    return
  }
...
function doRoleTip(bot, message, tipper, words, helpmsg, MultiorRole) {
  ...
  if (amount === null) {
    message
      .reply("I don't know how to tip that amount of SIRX...")
  ...
  if (roleToTip !== null) {
    ...
    if (membersOfRole.length > 0) {
      ...
      userIDs.forEach(u => {
        sendSIRX(bot, message, tipper, u, amount, isPrivateTip, MultiorRole)
      })
    } else {
      return message
    ...
    if (err) {
      ...
      sirius.sendFrom(tipper, address, Number(amount), 1, null, null, function(
        ...
        if (err) {
          ...
          let tx = txLink(txId)
          let msgtail = `
DM me with \`!tips\` for all available commands or read our Tipbot FAQ https://docs.getsirius.io/docs/faq/tipbot-discord for more details`
          if (privacyFlag) {
            let usr = message.guild.members.find('id', recipient).user
            let authmsg = `You have sent a private tip to @${
              usr.tag
            } with the amount of ${amount} SIRX.
${tx}${msgtail}`
            message.author.send(authmsg)
            if (message.author.id !== usr.id) {
              let recipientmsg = `You have just been privately tipped ${amount} SIRX by @${
                message.author.tag
              }.
${tx}${msgtail}`
            ...
            let generalmsg = `just tipped <@${recipient}> ${amount} SIRX.
${tx}${msgtail}`
...
function getValidatedAmount(amount) {
  amount = amount.toLowerCase().replace('sirx', '')
